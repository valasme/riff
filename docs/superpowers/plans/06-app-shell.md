...
export function Sidebar({ collapsed = false }: { collapsed?: boolean }) {
  ...
  return (
    <nav
      ...
      className={cn(
        "flex shrink-0 flex-col justify-between bg-surface py-3 transition-[width]",
  ...
  return (
    <li>
      <Link
        ...
        // Collapsed shows icons only, so the accessible name must come from
        // the attribute rather than the (now absent) visible text.
        aria-label={collapsed ? label : undefined}
        title={collapsed ? label : undefined}
        className={cn(
          "flex h-10 items-center gap-3 rounded-[var(--radius-nav)] px-3 text-[0.9375rem] font-medium",
